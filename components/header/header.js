...
function Header(props) {
  ...
  const switchChain = async () => {
    let hexChain = '0x'+Number(43114).toString(16)
    try {
      ...
      <div className={classes.chainInvalidError}>
        <div className={classes.ErrorContent}>
          ...
          <Typography className={classes.ErrorTxt}>
            The chain you're connected to isn't supported. Please check that your wallet is connected to Avax Mainnet.
          </Typography>
          <Button className={classes.switchNetworkBtn} variant="contained" onClick={()=>switchChain()} >Switch to { process.env.NEXT_PUBLIC_CHAINID == '4002' ? 'Avax Testnet' : 'Avalanche Mainnet' }</Button>
        </div>
      </div>
    ) : null}
    </div>
